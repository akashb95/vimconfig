...
vim.keymap.set(
  ...
  function() vim.cmd.RustLsp({ "renderDiagnostic", "current" }) end,
	{ silent = true, buffer = bufnr, desc = "[d]iagnostic float" }
)
vim.keymap.set(
  "n",
  "]d",
  function() vim.cmd.RustLsp({ "renderDiagnostic", "cycle" }) end,
	{ silent = true, buffer = bufnr, desc = "next [d]iagnostic" }
)
vim.keymap.set(
  "n",
  "[d",
  function() vim.cmd.RustLsp({ "renderDiagnostic", "cycle_prev" }) end,
	{ silent = true, buffer = bufnr, desc = "previous [d]iagnostic" }
)
vim.keymap.set(
  "n",
  "grd",
  function() vim.cmd.RustLsp("relatedDiagnostics") end,
	{ silent = true, buffer = bufnr, desc = "[g]o to [r]elated [d]iagnostics" }
)
...
vim.keymap.set(
  ...
  { silent = true, buffer = bufnr, desc = "[g]o e[x]pand [m]acros" }
)
